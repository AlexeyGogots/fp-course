...
filter f = foldRight (\a bs -> if f a then a :. bs else bs ) Nil
...
seqOptional =
  foldRight (\a as -> let f = mapOptional (:.) a in bindOptional (`mapOptional` as) f ) (Full Nil)
...
find predicate = foldRight (\a as -> if predicate a then Full a else as) Empty
-- find _ Nil = Empty
-- find predicate (a :. as) = if predicate a then Full a else find predicate as
